...
def setup():
    ...
    if not config.is_setup():
        ...
        if request.method == "POST":
            ...
            page = Pages(title=ctf_name, route="index", content="Sleep at night", draft=False)
...
def index_html():
    ...
    if  authed() is True:
        # User is logged in, so redirect or display a message instead of the account generation button
        logged_in_message = """
            <div style="text-align: center; margin-top: 20vh;">
                <p style="font-size: 18px; color: #333;">
                    You are already logged in. No need to generate a new account.
                </p>
            </div>
            """
        return render_template("page.html", content=logged_in_message, title="Welcome")
        
    button_html = """
    <div style="text-align: center; margin-top: 20vh;">
    <h2 style="margin-bottom: 20px; font-size: 24px; color: #333;">Create Your Account</h2>
    <form action="/" method="POST" style="display: inline-block; margin: auto;">
        <button type="submit" style="background-color: #2563EB; color: white; border: none; padding: 10px 20px; font-size: 16px; cursor: pointer; border-radius: 8px; transition: background-color 0.3s ease;">
            Generate Account
        </button>
    </form>
    </div>
    ...
    if request.method == "POST":
        ...
        email_address = f"{username}@example.com"  # Simple email generation for demonstration
        password = generate_password()

        # Auto-registration logic here
        # Assuming you have some defaults or auto-generated values for other fields
        website = None
        affiliation = "Auto-registered"
        country = "MY"  # Example default value
        bracket_id = None  # Example default value

        # Directly creating a user instance and adding it to your database
        user = Users(
            name=username,
            email=email_address,
        ...
        flash("Account created successfully! Please note down your details.", "success")
        account_details_html = f"""
        <div class="mt-4 p-5 bg-blue-100 rounded-lg">
            <h2 class="text-lg font-semibold">Account Created Successfully!</h2>
            <p><strong>Username:</strong> {username}</p>
            <p><strong>Password:</strong> {password}</p>
            <p>Please note down your details.</p>
            <p>btw, we've already logged in for you automatically.</p>
        </div>
        """

        return render_template("page.html", content=account_details_html, title="Account Generated")
    else:
        return render_template("page.html", content=button_html, title="Welcome")
